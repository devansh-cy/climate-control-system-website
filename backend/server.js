      ...
      if (!isProduction) {
        ...
        Product.find()
          .then(async (products) => {
            ...
            const hasLocalImages = products.some(p => p.images && p.images[0] && p.images[0].startsWith('/images/products/'));
            const { sampleProducts } = require('./seed');
            if (products.length !== sampleProducts.length || hasUnsplash || hasLocalImages) {
